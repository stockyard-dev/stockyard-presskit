...
func New(db *store.DB) *Server {
	...
	s.mux.HandleFunc("GET /api/assets", s.list)
	s.mux.HandleFunc("POST /api/assets", s.create)
	s.mux.HandleFunc("GET /api/assets/{id}", s.get)
	s.mux.HandleFunc("DELETE /api/assets/{id}", s.del)
...
func (s *Server) health(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"status": "ok", "service": "stockyard-presskit", "time": time.Now().UTC().Format(time.RFC3339)})
}

func (s *Server) stats(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"assets": s.db.Count()})
}

func (s *Server) list(w http.ResponseWriter, r *http.Request) {
	items := s.db.List()
	if items == nil { items = []store.Asset{} }
	wj(w, map[string]any{"assets": items, "count": len(items)})
}
