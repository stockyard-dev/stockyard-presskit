...
func New(db *store.DB, limits Limits, dataDir string) *Server {
	...
	s.mux.HandleFunc("POST /api/assets", s.createAsset)
	s.mux.HandleFunc("DELETE /api/assets/{id}", s.deleteAsset)
	s.mux.HandleFunc("GET /api/facts", s.listFacts)
	s.mux.HandleFunc("POST /api/facts", s.createFact)
	s.mux.HandleFunc("DELETE /api/facts/{id}", s.deleteFact)
	s.mux.HandleFunc("GET /api/contacts", s.listContacts)
	s.mux.HandleFunc("POST /api/contacts", s.createContact)
	s.mux.HandleFunc("DELETE /api/contacts/{id}", s.deleteContact)
	...
	s.mux.HandleFunc("GET /api/tier", func(w http.ResponseWriter, r *http.Request) {
		wj(w, 200, map[string]any{"tier": s.limits.Tier, "upgrade_url": "https://stockyard.dev/presskit/"})
	})
...
func (s *Server) listAssets(w http.ResponseWriter, r *http.Request) {
	wj(w, 200, map[string]any{"assets": oe(s.db.ListAssets())})
}
...
func (s *Server) deleteFact(w http.ResponseWriter, r *http.Request) {
	s.db.DeleteFact(r.PathValue("id"))
	wj(w, 200, map[string]string{"deleted": "ok"})
}
func (s *Server) listContacts(w http.ResponseWriter, r *http.Request) {
	wj(w, 200, map[string]any{"contacts": oe(s.db.ListContacts())})
}
...
func (s *Server) health(w http.ResponseWriter, r *http.Request) {
	st := s.db.Stats()
	wj(w, 200, map[string]any{"status": "ok", "service": "presskit", "assets": st.Assets})
}
func oe[T any](s []T) []T {
	if s == nil {
		return []T{}
...
func (s *Server) loadPersonalConfig() {
	...
	if err := json.Unmarshal(data, &cfg); err != nil {
		log.Printf("%s: warning: could not parse config.json: %v", "presskit", err)
		return
	}
	s.pCfg = cfg
	log.Printf("%s: loaded personalization from %s", "presskit", path)
}
